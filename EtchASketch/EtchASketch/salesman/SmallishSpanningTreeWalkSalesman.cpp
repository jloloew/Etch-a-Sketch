...
etchasketch::salesman::SmallishSpanningTreeWalkSalesman::
    SmallishSpanningTreeWalkSalesman(
        const std::unordered_set<KDPoint<2>> &unorderedPoints,
        const KDPoint<2> &startPoint)
    : Salesman(unorderedPoints, startPoint) {}

void etchasketch::salesman::SmallishSpanningTreeWalkSalesman::orderPoints() {
...
void etchasketch::salesman::SmallishSpanningTreeWalkSalesman::
    smallishSpanningTreeWalkAlgorithm(KDTree<2> &kdTree) {
  ...
  for (auto iter = unorderedPoints.begin(); iter != unorderedPoints.end();
       ++iter) {
  ...
  if (numComponents > 1) {
    ...
    for (auto iter = components.begin(); iter != components.end(); ++iter) {
      std::unordered_set<VertexDesc> *component = *iter;
      delete component;
    }
  }
  // Clean up.
...
void etchasketch::salesman::SmallishSpanningTreeWalkSalesman::connectComponents(
    UndirectedGraph &g, vector<GraphComponent *> &components) const {
  ...
  for (auto iter = components.begin(); iter != components.end(); ++iter) {
    ...
    if (!compCentersTree.insert(avgPoint)) {
      EASLog(
          "Error: center point already in use by another component. Skipping.");
    }
  }
...
KDPoint<2>
etchasketch::salesman::SmallishSpanningTreeWalkSalesman::findCenterPoint(
    const UndirectedGraph &g, const GraphComponent &comp,
...
etchasketch::salesman::SmallishSpanningTreeWalkSalesman::findNearestPoint(
    const UndirectedGraph &g, const GraphComponent &component,
...
void etchasketch::salesman::SmallishSpanningTreeWalkSalesman::mergeComponents(
    GraphComponent &dst, GraphComponent &src, const KDPoint<2> &centerDst,
    const KDPoint<2> &centerSrc, const UndirectedGraph &g,
    std::unordered_map<const KDPoint<2>, GraphComponent *> &componentCenters,
    std::unordered_map<const GraphComponent *, KDTree<2> *> &compTrees,
