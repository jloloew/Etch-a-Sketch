...
public:
	 ...
	 */
	KDTree(const std::vector< Point<Dim> > & newPoints);
...
private:
	 ...
	 */
	void buildTree(int a, int b, int dimension);
	
	/**
	 * Partition function for Quickselect. Performs a partition around
	 * the element points[pivotIndex].
	 * @param left the lower bound on the list
	 * @param right the upper bound on the list
	 * @param pivotIndex the index of the element to pivot around
	 * @param dimension the dimension to compare Point<Dim>s in
	 * @return the new index of the pivot element
	 */
	int partition(int left, int right, int pivotIndex, int dimension);
	
	/** Get the n-th smallest element of the list within left...right (inclusive). */
	int select(int left, int right, int n, int dimension);
